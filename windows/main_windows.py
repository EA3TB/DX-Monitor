#!/usr/bin/env python3
"""DX Monitor Windows — v1.0 — proceso único (monitor + web)"""
...
# ── Directorio de datos: junto al .exe (o junto al script en desarrollo) ──────
...
LOG_PATH    = os.path.join(DATA_DIR, "dx_monitor.log")
...
_LOG_STRINGS = {
    ...
    "xml_dir_missing":    {"es": "Directorio XML no configurado.",
                           "en": "XML directory not configured."},
    ...
    "spot_info":          {"es": "SPOT %s [%d] %s %s/%s -> %s",
                           "en": "SPOT %s [%d] %s %s/%s -> %s"},
    "alert_sent":         {"es": "ALERTA %s: %s [%d] %s %s/%s",
    ...
    # tray / main
    "tray_stop":          {"es": "Detenido desde bandeja del sistema.",
                           "en": "Stopped from system tray."},
    "startup":            {"es": "=== DX Monitor Windows %s ===",
                           "en": "=== DX Monitor Windows %s ==="},
    "data_dir":           {"es": "Datos en: %s",
                           "en": "Data directory: %s"},
    ...
    "dashboard_url":      {"es": "Dashboard en http://127.0.0.1:8765",
                           "en": "Dashboard at http://127.0.0.1:8765"},
}
...
# TimedRotatingFileHandler en Windows: backupCount con fechas en nombre — sin ":" en timestamps
_fh = logging.handlers.TimedRotatingFileHandler(
    LOG_PATH, when="W0", interval=1, backupCount=4, encoding="utf-8")
_fh.suffix = "%Y-%m-%d"   # evita los ":" que Windows no permite en nombres de fichero
...
def _write_json(path, data):
    ...
    # En Windows os.replace falla si el destino ya existe en algunas versiones antiguas;
    # usamos remove + rename como fallback seguro.
    try:
        os.replace(tmp, path)
    except OSError:
        try: os.remove(path)
...
def procesar_linea(linea, solo_registrar=False):
    ...
    if not modo_explicito:
        # Rango asimétrico: hasta -1 kHz por debajo y +3 kHz por encima de la frecuencia base.
        # Los usuarios a veces publican su frecuencia de TX (hasta +3 kHz sobre la base) sin indicar modo.
    ...
    # Spots históricos (sh/dx): registrar en antiduplicado pero no enviar alerta
    if solo_registrar:
    ...
    with _lock_alertas: _alertas_enviadas.add((call, banda, modo, ahora))
    log.info(_t("alert_sent"), tipo, call, dxcc_num, nombre, banda, modo)
    ...
    def hesc(s):
        """Escapa caracteres HTML para parse_mode=HTML de Telegram."""
        return str(s).replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")

    titulo = tg_label(tipo, lang)
    msg = ("📢 <b>%s</b>  —  %s\n📻 %.3f MHz  •  %s\n%s <b>%s — %s/%s</b>\n"
           "👁 %s: %s\n🕐 %s: %s\n🧭 Az SP: %d°  |  LP: %d°  |  %d km") % (
        hesc(call), hesc(nombre), freq, hesc(modo), icono, hesc(titulo), hesc(banda), hesc(modo),
        hesc(tg_label("spotter",lang)), hesc(spotter), hesc(hora_label), hesc(hora_str),
        az_sp, az_lp, dist)
    if clean:    msg += "\n📝 %s" % hesc(clean)
    if prop_str: msg += "\n📶 %s: %s" % (hesc(tg_label("propagacion",lang)), hesc(prop_str))
    ...
    entry = {"ts":now_str,"call":call,"dxcc":nombre,"freq":"%.3f"%freq,
             ...
             "spotter":spotter,"time_mode":time_mode}
    with _status_lock:
...
def bucle_cluster():
    while True:
        ...
        try:
            ...
            log.info(_t("cluster_auth") + " [%s]", cluster_type)
            with _status_lock:
            ...
            # Recoger la respuesta del sh/dx 20 por separado (timeout 3s sin nuevos datos)
            # y procesarla con solo_registrar=True para poblar el antiduplicado sin alertar
            ...
            while True:
                ...
                while "\n" in buf:
                    ...
                    if linea.strip():
                        ...
                        es_prompt = "CCC >" in linea or linea.endswith(">")
                        if not es_spot and not es_prompt: log.info("[CLUSTER] %s", linea)
...
def api_browse():
    """
    Explorador de directorios para Windows.
    ...
    # Seguridad: no subir por encima de una unidad de disco
    # (os.path.normpath ya resuelve ".." correctamente en Windows)
    try:
...
def _get_windows_drives():
    ...
    if sys.platform == "win32":
        ...
        # En desarrollo Linux/Mac: devolver "/" como única "unidad"
        drives = ["/"]
    return drives

def _parent_path(path):
    """Devuelve el directorio padre, o 'drives' si estamos en la raíz de una unidad."""
    if sys.platform == "win32":
        # Raíz de unidad: C:\ → volver a selector de unidades
        if os.path.splitdrive(path)[1] in ("\\", "/", ""):
            return "drives"
    parent = os.path.dirname(path)
    if parent == path:  # ya en la raíz absoluta
...
# ── Tray icon ─────────────────────────────────────────────────────────────────
def _crear_icono():
    """Carga icon.ico desde static/ si existe; si no, genera el icono con Pillow."""
    candidates = [os.path.join(BASE_DIR, "static", "icon.ico")]
    if getattr(sys, "frozen", False):
        candidates.insert(0, os.path.join(sys._MEIPASS, "static", "icon.ico"))
    for path in candidates:
        if os.path.exists(path):
            try:
                ico = Image.open(path)
                ico.load()
                return ico.convert("RGBA").resize((64, 64), Image.LANCZOS)
            except Exception:
                pass
    # Fallback generado con Pillow
    size = 64; s = 1.0
    img  = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    draw.rounded_rectangle([0, 0, size-1, size-1], radius=10, fill=(15, 21, 32, 255))
    draw.line([(32, 8), (32, 48)], fill=(200, 218, 240, 255), width=2)
    draw.line([(8,  8), (56,  8)], fill=(200, 218, 240, 255), width=3)
    draw.line([(14,22), (50, 22)], fill=(200, 218, 240, 255), width=2)
    draw.line([(20,36), (44, 36)], fill=(200, 218, 240, 255), width=2)
    draw.arc([ 5, 4, 17, 20], start=270, end=90,  fill=(0, 212, 255, 140), width=2)
    draw.arc([47, 4, 59, 20], start=90,  end=270, fill=(0, 212, 255, 140), width=2)
    draw.ellipse([48, 51, 58, 61], fill=(0, 204, 128, 255))
    draw.text((24, 53), "DX", fill=(0, 212, 255, 255))
    return img

def _abrir_dashboard(_icon=None, _item=None):
    webbrowser.open("http://127.0.0.1:8765")

def _detener(_icon, _item):
    log.info(_t("tray_stop"))
    _icon.stop()
    os._exit(0)

def _iniciar_tray():
    """Arranca el tray icon en el hilo principal tras ceder el control de Flask."""
    cfg      = leer_config()
    callsign = cfg.get("callsign","").upper().strip()
    title    = "DX Monitor — " + callsign if callsign else "DX Monitor"
    menu = pystray.Menu(
        pystray.MenuItem("Abrir dashboard", _abrir_dashboard, default=True),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem("Detener DX Monitor", _detener),
    )
    icon = pystray.Icon(
        name="DXMonitor",
        icon=_crear_icono(),
...
def _abrir_navegador():
    """Espera a que Flask arranque y abre el navegador por defecto."""
    time.sleep(1.5)
    webbrowser.open("http://127.0.0.1:8765")
...
def main():
    ...
    # Flask en hilo daemon — el hilo principal lo ocupa el tray icon
    flask_thread = threading.Thread(
        target=lambda: app.run(host="127.0.0.1", port=8765, debug=False,
                               use_reloader=False, threaded=True),
        daemon=True
    )
    flask_thread.start()

    # Tray icon en el hilo principal (requisito de pystray en Windows)
    _iniciar_tray()
